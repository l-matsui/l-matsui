...
public class PercolationTest {
    ...
    public void testOpenAndIsOpen() {
        ...
        for (int row = 0; row < 6; row++) {
            for (int col = 0; col < 6; col++) {
                assertThat(p.isOpen(row, col)).isFalse();
            }
        }
        p.open(0, 1);
        assertThat(p.isOpen(0, 1)).isTrue();
        p.open(5, 1);
        assertThat(p.isOpen(5, 1)).isTrue();
    }
    ...
    public void testIsFull() {
        ...
        p.open(5, 1);
        for (int row = 0; row < 6; row++) {
            assertThat(p.isFull(row, 1)).isTrue();
        }
    ...
    public void backwashTest() {
        ...
        assertThat(getState(3, p)).isEqualTo(expectedState);
        assertThat(p.percolates()).isTrue();
    }
