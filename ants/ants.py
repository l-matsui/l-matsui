...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            assert (place.ant == None or self.can_contain(place.ant) or place.ant.can_contain(self)), 'Two ants in {0}'.format(place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        def nearest_bee_place(place, away=0):
            if place.is_hive or away>self.max_range:
                return None
            elif place.bees and away>=self.min_range:
                return random_bee(place.bees)
            else:
                return nearest_bee_place(place.entrance, away+1)
        return nearest_bee_place(self.place)
    ...
    def action(self, gamestate):
        ...
        if self.is_slow:
            ...
            if self.slow_turns == 0:
                self.is_slow = False
            if gamestate.time % 2 == 0 and destination != None and self.health > 0:
            ...
            elif destination != None and self.health > 0:
                self.move_to(destination)
